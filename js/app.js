...
function renderSetup() {
  $("#setup-summary").innerHTML = `
    ...
    <div class="summary-row"><span>事務長</span><strong>${state.mode.emoji} ${state.mode.name}</strong></div>
...
function addBubble(role, text) {
  ...
  const label = role === "jimucho" ? "事務長" : "あなた";
...
function startSession() {
  state.engine = new JimuchoAI(state.scene.id, state.mode.id, state.theme.id);
