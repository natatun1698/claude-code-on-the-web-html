...
CATEGORIES = [
    # 「一般撮影（レントゲン）」は FPD/DR装置 等の汎用語を含み、他カテゴリの
    # 名称（例:「回診用X線撮影装置」「血管撮影システム」「X線テレビシステム」）
    # にも該当してしまうため、より特徴的な語を持つ他カテゴリを先に判定し、
    # 一般撮影は最後の消去法として判定する。
    ("回診用X線装置",
     ["回診用", "回診車", "移動型X線撮影", "ポータブル撮影", "ポータブルX線",
      "移動型汎用一体型X線", "汎用一体型X線透視診断装置"]),
    ("透視撮影台（X線テレビ）",
     ["X線テレビ", "X線TV", "透視撮影台", "据置型X線透視", "デジタル透視",
      "X線透視撮影装置", "CUEVISTA"]),
    ("血管撮影（CVS、アンギオ）",
     ["血管撮影", "アンギオ", "血管造影", "循環器X線", "X線循環器", "心血管X線",
      "バイプレーン", "CVS"]),
    ("外科用イメージ（可搬型Cアーム透視装置）",
     ["外科用イメージ", "外科用X線", "Cアーム", "移動型汎用X線透視",
      "移動型X線透視", "可搬型透視", "汎用X線透視診断装置"]),
    ("一般撮影（レントゲン）",
