/* =========================================================
 * engine.js — IA da contraparte (motor de diálogo baseado em regras)
 * e pontuação automática. Lógica compartilhada reutilizada por todos
 * os cenários em português (bucho-pt, etc.). O produto, os diálogos
 * e os textos de feedback de cada cenário ficam no data.js do cenário.
 * Globais que o data.js deve fornecer:
 ...
 *   event: "walkout" | "done" (cena concluída) | null
 * ========================================================= */
class RoleAI {
  ...
  /* Murmúrio de concordância (principalmente no modo A) */
  aizuchi() {
  ...
  respond(playerText) {
    ...
    /* --- 1. Modo C: preâmbulo longo -> interromper --- */
    ...
    /* --- 2. Jargão -> sempre perguntar de volta (mesmo termo só uma vez) --- */
    ...
    /* --- 3. Jogador ofereceu desconto -> pressionar de volta --- */
    ...
    /* --- 4. Recusou desconto e rebateu com valor -> reconhecer + avançar --- */
    let prefix = "";
    ...
    /* --- 5. Modo B: pedir esclarecimento uma vez em explicação confusa --- */
    if (
    ...
    /* --- 6. Modo C: resposta curta e direta restaura a confiança --- */
    if (this.mode.id === "C" && (a.conclusionFirst || a.len <= 60)) {
      this.strikes = Math.max(0, this.strikes - 1);
    }

    /* --- 7. Tema 2: aplicar pressão de desconto uma vez mesmo fora da cena de preço --- */
    if (
    ...
    /* --- 8. Cena de levantamento: só revela informação quando perguntado --- */
    ...
    /* --- 9. Cena normal: avançar para o próximo beat --- */
    ...
    /* --- 10. Quando os beats acabam: followup -> fechamento --- */
...
/* =========================================================
 * Pontuação automática — avalia apenas o tema escolhido.
...
function scoreSession(engine, walkout) {
  ...
  if (!turns.length) {
    result = {
      ...
      improvement: {
        point: `Você não disse nada. Comece dizendo apenas uma frase para o(a) ${ROLE_LABEL}.`,
        example: "\"Obrigado(a) pelo seu tempo hoje. Concluindo, ...\"",
      },
    };
  ...
  if (walkout) result.score = Math.max(0, result.score - 15); // penalidade por encerramento antecipado
...
function scoreT1(turns) {
  ...
  for (const t of turns) {
    ...
    for (const term of a.jargon) {
      jargonCount++;
      // penalidade reduzida pela metade se a mesma fala também tiver uma reformulação em linguagem simples
  ...
  if (!goodQuote) {
    // cita a fala mais longa, sem jargão, que pareça uma explicação
  ...
  return { score, grade: "", goodQuote, improvement, detail: `Jargão usado: ${jargonCount} vez(es)` };
}

/* --- Tema 2: rebater pedidos de desconto --- */
function scoreT2(turns) {
  let score = 60; // pontuação aditiva construída a partir dos pontos de valor oferecidos
  let offered = 0;
  ...
  turns.forEach((t, i) => {
    ...
    // dá mais peso à fala logo após a contraparte pressionar por desconto
  ...
  return { score, grade: "", goodQuote, improvement, detail: `Categorias de valor: ${catsUsed.size} / Menções a desconto: ${offered}` };
}

/* --- Tema 3: ir direto à conclusão, com concisão --- */
function scoreT3(turns) {
  ...
  const improvement = interrupted || longCount
    ...
    : FEEDBACK.t3.good();
  return { score, grade: "", goodQuote, improvement, detail: `Tamanho médio: ${Math.round(avg)} caracteres / Vezes interrompido: ${interrupted}` };
}

/* --- Tema 4: captar a intenção da pergunta --- */
function scoreT4(turns) {
  ...
  return { score, grade: "", goodQuote, improvement, detail: `Perguntas respondidas corretamente: ${hit}/${asked}` };
}
